...
public class MockAiDetectionService : IAiDetectionService
{
    ...
    /// <summary>
    /// تحليل صوتي للكشف عن الذكاء الاصطناعي
    /// </summary>
    public async Task<AiDetectionResult> DetectAudioAsync(string audioPath)
    {
        var stopwatch = Stopwatch.StartNew();
        
        await Task.Delay(120); // محاكاة وقت المعالجة

        var indicators = new List<string>();
        double aiScore = 0.3; // قيمة أساسية
        ...
        try
        {
            ...
            {
                aiScore += 0.1;
                indicators.Add($"حجم الملف ({fileSizeMb:F1} MB) معتاد لملفات الصوت القصيرة");
            }
            ...
            {
                aiScore += 0.05;
                indicators.Add($"صيغة {extension.ToUpper()} شائعة في أصوات AI");
            }

            // 3. إضافة عشوائية بناءً على اسم الملف
            var random = new Random(audioPath.GetHashCode());
            var randomFactor = (random.NextDouble() - 0.5) * 0.4; // عشوائية أكبر قليلاً للأصوات
            ...
            return new AiDetectionResult
            {
                ...
                IsAiGenerated = finalProbability > 0.5,
                Confidence = confidence,
                Details = $"تم تحليل الملف الصوتي ({fileSizeMb:F1} MB). " +
                         $"احتمالية AI: {finalProbability:P0}. " +
                         (finalProbability > 0.7 ? "مؤشرات قوية على صوت مُنشأ بالذكاء الاصطناعي." :
                          finalProbability > 0.5 ? "مؤشرات متوسطة على صوت مُنشأ بالذكاء الاصطناعي." :
                          "المقاطع الصوتية تبدو طبيعية بنسبة كبيرة."),
                Indicators = indicators,
                ProcessingTimeMs = stopwatch.ElapsedMilliseconds
            };
        }
        catch (Exception ex)
        {
            ...
            return new AiDetectionResult
            {
                ...
                IsAiGenerated = false,
                Confidence = 0,
                Details = $"خطأ في تحليل الصوت: {ex.Message}",
                ProcessingTimeMs = stopwatch.ElapsedMilliseconds
            };
        }
    }
